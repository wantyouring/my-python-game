...
def playgame(gamepad,man,ddong,clock,agent):
    ...
    # 초기 똥 추가
    for i in range(TOTAL_DDONG):
        ddong_x.append(random.randrange(0,12) * 40) # 특정 랜덤 위치에서 똥 떨어짐.
        ddong_y.append(random.randrange(0,16) * -40)
    ...
    while not end_game:
        ...
        for index, value in enumerate(ddong_y):
            ...
            if value > PAD_HEIGHT:
                ddong_x[index] = random.randrange(0,12) * 40 # x좌표 random위치 생성
...
if __name__ == "__main__":
    ...
    for e in range(EPISODES):
        if e % 1000 == 0:
            graph_cnt += 2
            episodes, scores, scores30, avg_q_max_record = [],[],[], []
